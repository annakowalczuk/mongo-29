...
const updateUsername = function () {
  // update username
  return User.findOneAndUpdate({username: 'Benny_the_boy'}, {username: 'Benny_the_man'}, {new: true}, 
    function (err, user) {
      if (err) throw err;
      if (user) console.log('Nazwa uzytkownika po aktualizacji to ' + user.username);
  })
}
...
var prom1 = Promise.all([kenny.save(), mark.save(), benny.save()])
  .then(findAllUsers)
